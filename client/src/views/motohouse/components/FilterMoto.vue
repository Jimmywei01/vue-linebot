          ...
          <el-col :span="12" class="motoCatalog_col">
            <el-form-item class="motoCatalog_container">
              <el-select label='車款' placeholder="車款"  v-model="motoCatalog" clearable  @change="getMotoCatalog">
                <el-option  v-for="item in motoCatalogOptions" :key="item.id" :label="item.motoCatalog" :value="item"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
        </el-row>
  ...
  */
  methods: {
    ...
    async getMotoStore (value) {
      ...
      await this.$store.commit('motoHouse/GET_FILTER_MOTOCATALOG', value)
      this.motoCatalogOptions = await this.$store.getters.seletMotosCatalog
    ...
    */
    async updatfilterData (value) {
      ...
      this.getfilterData()
      this.motoStoreOptions = this.$store.getters.seletMotos
      this.motoCatalogOptions = this.$store.getters.seletMotosCatalog
      // console.log('newMotoStoreOptions:', this.motoStoreOptions)
      // console.log('newMotoCatalogOptions:', this.motoCatalogOptions)
    },
    async deleteOption (value) {
      ...
      this.motoStoreOptions = this.$store.getters.seletMotos
      this.motoCatalogOptions = this.$store.getters.seletMotosCatalog
      // console.log('deleteMotoStoreOptions:', this.motoStoreOptions)
      // console.log('deleteMotoCatalogOptions:', this.motoCatalogOptions)
    }
  }
}
</script>
