  ...
  <div class="motoDetailArea">
    ...
    <section class="form_container">
      <el-table type="flex" justify="center" align="middle" :data="tableData" max-height="450">
        <el-table-column label="廠商" align="center" width="130">
          <template slot-scope="{ row }">
            <p>{{ row.motoStore }}</p>
          </template>
        </el-table-column>

        <el-table-column label="排氣量" align="center" width="130">
          <template slot-scope="{ row }">
            <p>{{ row.cc }}</p>
          </template>
        </el-table-column>

        <el-table-column label="車款" align="center" width="130">
          <template slot-scope="{ row }">
            <p>{{ row.motoCatalog }}</p>
          </template>
        </el-table-column>

        <el-table-column label="引擎" align="center" width="180">
          <template slot-scope="{ row }">
            <p>{{ row.engine }}</p>
          </template>
        </el-table-column>

        <el-table-column label="變速" align="center" width="130">
          <template slot-scope="{ row }">
            <p>{{ row.variable }}</p>
          </template>
        </el-table-column>

        <el-table-column label="油箱" align="center" width="130">
          <template slot-scope="{ row }">
            <p>{{ row.oil }}</p>
          </template>
        </el-table-column>

        <el-table-column label="煞車" align="center" width="130">
          <template slot-scope="{ row }">
            <p>{{ row.brake }}</p>
          </template>
        </el-table-column>
        ...
        <el-table-column label="Actions" align="center" width="180">
          <template slot-scope="{ row }">
            ...
            <DialogFound
              :dialog="dialog"
              :form="form"
              @update:updateData="getMotos"
              @update:updateLoading="getLoading"
            ></DialogFound>
      ...
      <template>
        <el-button type="primary" icon="el-icon-plus" round class="buttonPlus" @click="handleAdd"> </el-button>
...
import DialogFound from '@/components/DialogFound/motoDetailDF.vue'
...
export default {
  ...
  methods: {
    ...
    async getMotos (value) {
      ...
      if (value) {
        ...
        if (this.filterStore) {
          ...
          await this.$store.commit('motoDetail/SET_FILTER_MOTOSTORE', this.filterStore)
          await this.$store.commit('motoDetail/GET_FILTER_MOTOCATALOG', this.filterStore)

          this.tableData = await this.$store.getters.seletMotosDetail
          ...
          console.log('未篩選')
          this.tableData = await this.$store.getters.motosDetail
        }
        ...
        if (this.filterStore) {
          ...
          await this.$store.commit('motoDetail/SET_FILTER_MOTOSTORE', this.editDeleteOption.motoStore)
          await this.$store.commit('motoDetail/GET_FILTER_MOTOCATALOG', this.editDeleteOption.motoCatalog)
          this.tableData = await this.$store.getters.seletMotosDetail
    ...
    async handleDelete (row) {
      console.log('刪除:', row)
      const formDefault = 'motoDetail/deleteMoto'
      const formLine = 'motoDetail/deleteMotoLine'
      const changeForm = this.user.password ? formLine : formDefault
      this.$store
        .dispatch(changeForm, row.id)
        .then((res) => {
          this.$swal.fire({
            icon: 'success',
            title: '刪除成功'
          ...
          this.$store.commit('motoDetail/DELETE_MOTO', row.id)
          const newMotos = this.$store.getters.motosDetail
          // console.log('newMotos:', newMotos)
          this.$store.commit('motoDetail/GET_FILTER_MOTOSTORE', newMotos)
          this.$store.commit('motoDetail/GET_FILTER_MOTOCATALOG', row.motoStore)
    ...
    async handleFilterMotoCatalog (value) {
      ...
      if (this.tableData.length < 1) {
        ...
        if (updateValue.length < 1) {
          ...
          await this.$store.commit('motoDetail/SET_FILTER_MOTOSTORE', updateValue[0].motoStore)
          await this.$store.commit('motoDetail/GET_FILTER_MOTOCATALOG', updateValue[0].motoStore)
          await this.$store.commit('motoDetail/SET_FILTER_MOTOCATALOG', value)
          this.tableData = await this.$store.getters.seletMotosDetailCatalog
        } else if (updateValue.length) {
          console.log('進這:', updateValue.length)
          await this.$store.commit('motoDetail/UPDATE_FILTER_MOTO', updateValue)
          await this.$store.commit('motoDetail/GET_FILTER_MOTOCATALOG', this.filterStore)
          await this.$store.commit('motoDetail/SET_FILTER_MOTOCATALOG', value)
          this.tableData = await this.$store.getters.seletMotosDetailCatalog
        }
